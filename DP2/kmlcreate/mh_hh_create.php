...
$NetStyleNode->setAttribute('id', 'HHstyle');
...
$NetHref = $dom->createElement('href', 'http://ossportal/out_dated/SLTIlastrator/images/HH.png');
...
$NetStyleNode = $dom->createElement('Style');
$NetStyleNode->setAttribute('id', 'MHstyle');
$NetIconstyleNode = $dom->createElement('IconStyle');
$NetIconstyleNode->setAttribute('id', 'msan');
$Netscale=$dom->createElement('scale', '0.95');
$NetIconstyleNode->appendChild($Netscale);
$NetIconNode = $dom->createElement('Icon');
$NetHref = $dom->createElement('href', 'http://ossportal/out_dated/SLTIlastrator/images/MH.png');
$NetIconNode->appendChild($NetHref);
$NetIconstyleNode->appendChild($NetIconNode);
$NetStyleNode->appendChild($NetIconstyleNode);
$docNode->appendChild($NetStyleNode);

$sql="SELECT  LATITUDE,LONGITUDE,LOCATION,EQ_INDEX  FROM OSSRPT.GPS_LOCATION  where TYPE='MH_HH' and RTOM ='$rtom'";
...
while ($row = oci_fetch_array($res)) {
  ...
  if($row['EQ_INDEX'] == 'H')
  {
    ...
    $styleUrl = $dom->createElement('styleUrl', '#HHstyle');
    $placeNode->appendChild($styleUrl);

  }else{

    $styleUrl = $dom->createElement('styleUrl', '#MHstyle');
    $placeNode->appendChild($styleUrl);
