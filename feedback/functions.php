...
if($q == 'region'){
	...
	if($r=='ALL'){
		...
		$y = str_replace('*','&',$r);
	$sql = "SELECT DISTINCT PROVINCE  FROM OSSRPT.SLT_AREA  WHERE REGIONS like '$y'";
	$result = "ALL@";
	}
...
if($q == 'province'){
	$result = null;
	if($r=='ALL'){
		$sql = "SELECT DISTINCT RTOM_CODE  FROM OSSRPT.SLT_AREA  WHERE REGIONS like '$s'";
...
if($q == 'province2'){
	$result = null;
	if($r=='ALL'){
		$sql = "SELECT DISTINCT OPMC_NAME  FROM OSSRPT.SLT_AREA  WHERE REGIONS like '$s'";
...
oci_free_statement($stid);
		...
		$y = str_replace('*','&',$r);
	$sql = "SELECT NVL(DISCRIPTION,'PENDING') , NVL(COUNT(DISTINCT B.FAULT_ID),0) FROM SMS_CALLBACK A ,(
SELECT *  FROM SMS_FEEDBACK
WHERE LEA IN (SELECT  LEA_CODE FROM OSSRPT.SLT_AREA $y AND SMS_DATE $day
AND FEEDBACK IN (1,2,3) ) B
WHERE A.FAULT_ID(+) = B.FAULT_ID
GROUP BY DISCRIPTION
ORDER BY NVL(COUNT(B.FAULT_ID),0)";
...
,C.CIRT_DISPLAYNAME,STATUS,IN_DATE,LEA,SMS_DATE
...
while(oci_fetch($stid)){
 ...
 .",".oci_result($stid,oci_field_name($stid, 11)).",".oci_result($stid,oci_field_name($stid, 12)).",".oci_result($stid,oci_field_name($stid, 14)).",".oci_result($stid,oci_field_name($stid, 15))."@"; 
}
